...
$ ambari-server start
$ curl http://c7301.ambari.apache.org:8080         (启动需要几分钟，用户名口令是admin/admin)
$ ping u1401                     (可以看到u1401的ip是192.168.14.101)
```
在windows下的浏览器中输入```http://c7301.ambari.apache.org:8080```就可以调出ambari的界面，初始用户名口令是`admin/admin`。  
